...
import sys
sys.path.append('/mnt/cache/liyi/WSS')
import tool.data
...
if __name__ == '__main__':
    ...
    parser.add_argument("--structure", default="data/ckpt/scalenet101.json", type=str)
    ...
    if args.network == "ScaleNet50_Aff":
        from network.scalenet_aff import ScaleNet50_Aff
        model = ScaleNet50_Aff(args.structure, dilations=[int(i) for i in args.dilations.split('_')])
    elif args.network == "ScaleNet101_Aff":
        from network.scalenet_aff import ScaleNet101_Aff
        model = ScaleNet101_Aff(args.structure, dilations=[int(i) for i in args.dilations.split('_')])
    else:
        model = getattr(importlib.import_module(args.network), 'Net')()

    print(model)

    model.radius = args.radius

    train_dataset = tool.data.VOC12AffDataset(args.train_list, label_la_dir=args.la_crf_dir, label_ha_dir=args.ha_crf_dir,
    ...
    if args.weights[-7:] == '.params':
        ...
        state_dict = network.resnet38d.convert_mxnet_to_torch(args.weights)
    else:
    ...
    for k, v in empty_state_dict.items():
        try:
            if v.shape != state_dict[k].shape:
                v[:state_dict[k].shape[0], :state_dict[k].shape[1]].copy_(state_dict[k])
            else:
